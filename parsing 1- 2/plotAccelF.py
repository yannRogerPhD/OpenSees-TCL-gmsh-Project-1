...
def accelPlot():
    ...
    plt.figure(1)
    plt.plot(time, a[:, nNode - 1, 0], '-b', linewidth=0.85)
    ...
    ax1.semilogx(p, aMax, 'b', linewidth=2.5)
    ax1.grid(True, which="minor", linestyle="--", linewidth=0.5)  # log-type grid
    # ax1.grid(True, which="both", linestyle="--", linewidth=0.5)  # log-type grid
    ax1.set_ylabel('$\\text{S}_a (g)$', fontsize=20)
    ax1.set_xticklabels([])
    ax1.tick_params(axis='both', which='major', labelsize=16)

    ax2.semilogx(p, vMax, 'b', linewidth=2.5)
    ax2.grid(True, which="minor", linestyle="--", linewidth=0.5)  # log-type grid
    ax2.set_ylabel('$\\text{S}_v$ (m/s)', fontsize=20)
    ax2.set_xticklabels([])
    ax2.tick_params(axis='both', which='major', labelsize=16)

    ax3.semilogx(p, uMax, 'b', linewidth=2.5)
    ax3.grid(True, which="minor", linestyle="--", linewidth=0.5)  # log-type grid
