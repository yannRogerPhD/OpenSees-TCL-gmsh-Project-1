...
FuzzyFloat, selectNodes, defaultTol, _roundFunc, soilFaceNodesAroundPile, detectSoilGroups, classifySoilAndPileNodes,
groupNodesByCoordinate, buildSSImap, getAndSortGroupNodes)
...
print(f"\nTotal nodes found: {len(nodeDOFs)}\n  2-DOF nodes: {len(twoDOFNodes)}\n  3-DOF nodes: {len(threeDOFNodes)}")
...
print("[DEBUG] all pile nodes sorted by Y:", sortNodesByY(pileNodeSet, nodeCoords))

tol = defaultTol
soilByY = groupNodesByCoordinate(soilNodeSet, nodeCoords, axis="y", tol=tol)
pileByY = groupNodesByCoordinate(pileNodeSet, nodeCoords, axis="y", tol=tol)

print(f"[SSI] z-layers with soil: {len(soilByY)}\n[SSI] z-layers with pile: {len(pileByY)}\n")

# SSI_map: pileNode --> list of surrounding soil node IDs
SSI_map = buildSSImap(pileNodeSet, elements, soilTypes, nodeCoords)
