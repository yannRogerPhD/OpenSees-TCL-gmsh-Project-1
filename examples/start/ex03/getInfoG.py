...
groupL, groupR, groupB, groupLB, groupRB, groupM = 5, 3, 1, 4, 2, 6
phyGroups = []

nodeCoords = {}
nodeDOFs = {}

leftX, rightX, bottomY = -130, 130, -20
...
titleL, titleR, titleRB, titleLB, titleM, titleB = False, False, False, False, False, False
                    ...
                    if len(parts) == 9:
                        try:
                            ...
                            if phyGroup == groupL:
                                ...
                                bType = 'L'
                                if not titleL:
                                    f_ele.write("\n# !!!!! for left absorbing boundary elements !!!!!\n")
                                    titleL = True
                                ...
                                absElements.add(eleTag)

                            elif phyGroup == groupR:
                                E = 3.0e9
                                poisson = 0.3
                                rho = 2100
                                thickness = 1.0
                                ...
                                bType = 'R'
                                if not titleR:
                                    f_ele.write("\n# !!!!! for right absorbing boundary elements !!!!!\n")
                                    titleR = True
                                f_ele.write(f"element "
                                            f"ASDAbsorbingBoundary2D "
                                            f"{eleTag} "
                                ...
                                bType = 'B'
                                if not titleB:
                                    f_ele.write("\n# !!!!! for bottom absorbing boundary elements !!!!!\n")
                                    titleB = True
                                f_ele.write(f"element "
                                            f"ASDAbsorbingBoundary2D "
                                            f"{eleTag} "
                                ...
                                bType = 'LB'
                                if not titleLB:
                                    f_ele.write("\n# !!!!! for bottom left absorbing boundary elements !!!!!\n")
                                    titleLB = True
                                f_ele.write(f"element "
                                            f"ASDAbsorbingBoundary2D "
                                            f"{eleTag} "
                                ...
                                bType = 'RB'
                                if not titleRB:
                                    f_ele.write("\n# !!!!! for bottom right absorbing boundary elements !!!!!\n")
                                    titleRB = True
                                f_ele.write(f"element "
                                            f"ASDAbsorbingBoundary2D "
                                            f"{eleTag} "
                                ...
                                bType = 'RB'
                                if not titleM:
                                    f_ele.write("\n# !!!!! main soil elements: for principal FE soil elements !!!!!\n")
                                    titleM = True
                                f_ele.write(f"element "
                                            f"quad "
                                            f"{eleTag} "
...
output_UpdateASD = 'updateASD.tcl'

if absElements:
    with open(output_UpdateASD, 'w') as outfileAbsorbing:
        for i in absElements:
            # print(f"setParameter -val 1 -ele {i} stage")
            outfileAbsorbing.write(f"setParameter -val 1 -ele {i} stage\n")
