...
ASDLeftGrp, ASDBottomGrp, ASDRightGrp, ASDBottomLeftGrp, ASDBottomRightGrp = {}, {2}, {}, {}, {}
...
leftNodes = getBoundaryNodesFromMsh(meshFile, phyGroupID=7, dim=1)
rightNodes = getBoundaryNodesFromMsh(meshFile, phyGroupID=5, dim=1)
...
with open(outputPath, 'w') as f:
    for i, j in zip(leftNodes[1:], rightNodes[1:]):
        f.write(f"equalDOF {i} {j} 1 2\n")

ASDElements = [el["id"] for el in elements if el["type"] in {10031, 10032, 10033, 10034, 10035}]

outputPathASD = os.path.join(outDir, 'updateASDElements.tcl')
with open(outputPathASD, 'w') as f:
    for i in ASDElements:
        f.write(f"setParameter -val 1 -ele {i} stage\n")