...
def emit_geo(
    ...
    if symmetries_:
        ...
        for k, sp in enumerate(symmetries_, start=1):
            L_.append(
                f"dup{k}[] = Symmetry {{{_fmt(sp.a)}, {_fmt(sp.b)}, {_fmt(sp.c)}, {_fmt(sp.d)}}} "
                f"{{ Duplicata {{ Volume{{curVols[]}}; }} }};"
            )
            L_.append(f"curVols[] += dup{k}[];")
            L_.append("curVols[] = Unique(Abs(curVols[]));")
        L_.append("")
    ...
    # ---- reusable transfinite constraints macro (NEW) ----
    L_.append("// Reusable curve-constraint macro (keeps .geo small)")
    L_.append("Macro ApplyBBoxCurveConstraints")
    L_.append("  xMin = (X0 - Lx/2); xMax = (X0 + Lx/2);")
    L_.append("  yMin = (Y0 - Ly/2); yMax = (Y0 + Ly/2);")
    L_.append("  zMin = (Z0 - Lz  ); zMax = (Z0      );")
    L_.append("  cX[] = {}; cY[] = {}; cZ[] = {};")
    L_.append("  // X edges")
    L_.append("  cX[] += Curve In BoundingBox { xMin-eps, yMin-eps, zMin-eps, xMax+eps, yMin+eps, zMin+eps };")
    L_.append("  cX[] += Curve In BoundingBox { xMin-eps, yMin-eps, zMax-eps, xMax+eps, yMin+eps, zMax+eps };")
    L_.append("  cX[] += Curve In BoundingBox { xMin-eps, yMax-eps, zMin-eps, xMax+eps, yMax+eps, zMin+eps };")
    L_.append("  cX[] += Curve In BoundingBox { xMin-eps, yMax-eps, zMax-eps, xMax+eps, yMax+eps, zMax+eps };")
    L_.append("  // Y edges")
    L_.append("  cY[] += Curve In BoundingBox { xMin-eps, yMin-eps, zMin-eps, xMin+eps, yMax+eps, zMin+eps };")
    L_.append("  cY[] += Curve In BoundingBox { xMin-eps, yMin-eps, zMax-eps, xMin+eps, yMax+eps, zMax+eps };")
    L_.append("  cY[] += Curve In BoundingBox { xMax-eps, yMin-eps, zMin-eps, xMax+eps, yMax+eps, zMin+eps };")
    L_.append("  cY[] += Curve In BoundingBox { xMax-eps, yMin-eps, zMax-eps, xMax+eps, yMax+eps, zMax+eps };")
    L_.append("  // Z edges")
    L_.append("  cZ[] += Curve In BoundingBox { xMin-eps, yMin-eps, zMin-eps, xMin+eps, yMin+eps, zMax+eps };")
    L_.append("  cZ[] += Curve In BoundingBox { xMin-eps, yMax-eps, zMin-eps, xMin+eps, yMax+eps, zMax+eps };")
    L_.append("  cZ[] += Curve In BoundingBox { xMax-eps, yMin-eps, zMin-eps, xMax+eps, yMin+eps, zMax+eps };")
    L_.append("  cZ[] += Curve In BoundingBox { xMax-eps, yMax-eps, zMin-eps, xMax+eps, yMax+eps, zMax+eps };")
    L_.append("  cX[] = Unique(Abs(cX[])); cY[] = Unique(Abs(cY[])); cZ[] = Unique(Abs(cZ[]));")
    L_.append("  Transfinite Curve {cX[]} = TX Using Progression 1;")
    L_.append("  Transfinite Curve {cY[]} = TY Using Progression 1;")
    L_.append("  Transfinite Curve {cZ[]} = TZ Using Progression 1;")
    L_.append("Return")
    L_.append("")
    ...
    # Curve constraints for every symmetry image of every seed box (COMPACT)
    for i, b in enumerate(boxes_, start=1):
        # Keep pre-seed size symbols (as before)
        L_.append(f"Lx_{i} = {_fmt(b.Lx)}; Ly_{i} = {_fmt(b.Ly)}; Lz_{i} = {_fmt(b.Lz)};")

        # Compute all instance centers in Python (seed and sequential reflections)
        inst = _all_instance_centers([b], symmetries_)  # returns (x0,y0,z0,BoxSpec)
        for (x0, y0, z0, bb) in inst:
            # Set globals consumed by the macro
            L_.append(f"X0 = {_fmt(x0)}; Y0 = {_fmt(y0)}; Z0 = {_fmt(z0)};")
            L_.append(f"Lx = Lx_{i}; Ly = Ly_{i}; Lz = Lz_{i};")
            L_.append(f"TX = {bb.transX}; TY = {bb.transY}; TZ = {bb.transZ};")
            L_.append("Call ApplyBBoxCurveConstraints;")
        L_.append("")
    ...
    # L_.append("allVols[] = Volume{;};")
    L_.append("allVols[] = Volume{:};")
    L_.append("allVols[] = Unique(Abs(allVols[]));")
...
if __name__ == "__main__":
    ...
    layers = [
        dict(z_top=0.00, thickness=5.0*2.0, ZMeshSize=2.0, transZ="transZ_L1"),  # 1.25
        dict(z_top=-5*2, thickness=4.0*1.5, ZMeshSize=1.5, transZ="transZ_L2"),  # 1.5
        dict(z_top=-(5*2 + 4*1.5), thickness=5*1, ZMeshSize=1.0, transZ="transZ_L3"),  # 1.25
